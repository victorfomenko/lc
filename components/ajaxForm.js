...
export default class AjaxForm extends Component {
  componentDidMount() {
        ...
        if (data.status === 'ok') {
          ...
          if (data.code === 'main') {
            if ($mainErrorContainer !== null) {
              $mainErrorContainer.html('<span class="m-text_error">' + data.message + '</span>');
            } else {
              $form.append('<span class="m-text_error">' + data.message + '</span>');
            }
          } else {
    ...
    script.ajaxform.init(this.node);
  ...
  render () {
    return <form {...this.props} ref={node=>this.node=node}>{this.props.children}</form>
  }
}